  ...
  resetOrganizationsForm(orgId) {
    ...
    authRequest(`/api/organizations/${orgId}`).then((response) => {
      ...
      if (data.photo_url) {
        ...
        document.getElementById('organization_photo_url').src = "/static/dashboard/images/account/organization.svg";
      }
      if (data.public) {
...
function addTeamMemberCard(orgId, claims, data, gridId = 'team-member-grid') {
  ...
  <div class="d-flex align-items-center">
    ...
    <button class="btn btn-sm-light text-danger" onclick="cannabisdata.settings.removeTeamMember('${data.uid}')">
      Remove
    </button>
  </div>`
  }
